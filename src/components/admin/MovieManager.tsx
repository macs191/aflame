    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <div className="glass-card p-8 rounded-2xl h-fit">
        <h2 className="text-xl font-bold mb-6 text-gold-400 flex items-center gap-2">
          <Plus className="w-5 h-5" /> إضافة فيلم جديد
        </h2>
        ...
        <form onSubmit={handleAdd} className="space-y-5">
          ...
          <Field label="رابط البث (HLS .m3u8)" value={streamUrl} onChange={setStreamUrl} required type="url" ltr placeholder="https://example.com/movie.m3u8" />
      ...
      <div className="glass-card p-8 rounded-2xl">
        <h2 className="text-xl font-bold mb-6 text-gold-400 flex items-center gap-2">
          <Film className="w-5 h-5" /> الأفلام الحالية ({movies.length})
        </h2>
        {loadingList ? (
          ...
          <p className="text-gray-400 text-center py-10">لا توجد أفلام مضافة بعد.</p>
        ) : (
          <ul className="space-y-3 max-h-[32rem] overflow-y-auto pr-1">
            {movies.map((m) => (
              <li key={m.id} className="flex items-center gap-3 bg-dark-900/50 border border-gold-500/10 rounded-xl p-3">
                <img src={m.posterUrl} alt={m.titleAr} className="w-12 h-16 object-cover rounded-lg bg-dark-800 shrink-0" loading="lazy" />
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-white truncate">{m.titleAr}</p>
                  <p className="text-xs text-gray-400 flex items-center gap-2 mt-1">
                    <span className="flex items-center gap-1 text-gold-500"><Star className="w-3 h-3 fill-gold-500" /> {m.rating}</span>
                    <span>{m.releaseYear}</span>
                    <span>{m.durationMinutes} د</span>
                  </p>
                </div>
                <button onClick={() => handleDelete(m.id)} className="p-2 text-red-400 hover:bg-red-500/10 rounded-lg transition-colors shrink-0" aria-label="حذف الفيلم">
