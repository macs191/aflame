...
import { Clapperboard, Star, Calendar, Loader2, Play, Layers, X, Tv } from 'lucide-react';
...
export const Series: React.FC = () => {
  ...
  const handleSelectSeries = async (series: SeriesType) => {
    ...
    try {
      ...
      console.error('فشل في جلب حلقات المسلسل:', err);
    } finally {
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        <div className="flex items-center justify-between border-b border-gold-500/20 pb-4">
          <h1 className="text-3xl font-extrabold text-gold-500 flex items-center gap-3">
            <Clapperboard className="w-8 h-8" /> مكتبة المسلسلات
          </h1>
            ...
            <div className="relative w-full max-w-6xl bg-dark-800 rounded-2xl p-6 border border-gold-500/30 space-y-6 my-auto">
              ...
              {activeEpisode && activeEpisode.streamSources.length > 0 ? (
                <div className="space-y-3">
                  <VideoPlayer
                    source={activeEpisode.streamSources[0]}
                    posterUrl={activeEpisode.thumbnailUrl || selectedSeries.bannerUrl}
                  />
                  <div className="flex items-center justify-between">
                    <h3 className="text-xl font-bold text-gold-400">
                      الحلقة {activeEpisode.episodeNumber}: {activeEpisode.titleAr}
                    </h3>
                  </div>
                </div>
              ) : (
                <div className="aspect-video bg-dark-900 rounded-2xl flex items-center justify-center text-gray-400">
                  {loadingEpisodes ? (
                    <Loader2 className="w-10 h-10 animate-spin text-gold-500" />
                  ) : (
                    'اختر حلقة للبدء في المشاهدة'
                  )}
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4 border-t border-gold-500/10">
                <div className="md:col-span-1 space-y-3">
                  ...
                  <p className="text-gray-400 text-sm leading-relaxed">{selectedSeries.descriptionAr}</p>
                  <div className="flex items-center gap-4 text-xs text-gray-400 pt-2">
                    <span className="flex items-center gap-1 text-gold-500">
                      <Star className="w-4 h-4 fill-gold-500" /> {selectedSeries.rating}
                    </span>
                    <span className="flex items-center gap-1">
                      <Layers className="w-4 h-4" /> {selectedSeries.totalSeasons} مواسم
                    </span>
                  </div>
                </div>
                ...
                <div className="md:col-span-2 space-y-3">
                  ...
                  {loadingEpisodes ? (
                    <div className="py-8 text-center text-gray-400">جاري تحميل الحلقات...</div>
                  ) : episodes.length > 0 ? (
                        ...
                        <button
                          ...
                          className={`p-3 rounded-xl border text-right transition-all flex flex-col justify-between ${
                            activeEpisode?.id === ep.id
                              ...
                              : 'border-gold-500/10 bg-dark-900/50 hover:border-gold-500/30 text-gray-300'
                          }`}
                        >
                          <span className="text-xs text-gold-500 mb-1">حلقة {ep.episodeNumber}</span>
                          <span className="text-sm line-clamp-1">{ep.titleAr}</span>
                        </button>
                      ))}
                    </div>
                  ) : (
        ...
        {seriesList.length > 0 ? (
              ...
              >
                <div className="relative aspect-[2/3] overflow-hidden bg-dark-800">
                  ...
                  <div className="absolute top-3 right-3 bg-dark-900/80 backdrop-blur-md text-gold-400 text-xs font-bold px-2.5 py-1 rounded-lg border border-gold-500/20 flex items-center gap-1">
                    <Star className="w-3 h-3 fill-gold-400" /> {series.rating}
                  </div>
                </div>
                ...
                <div className="p-4 flex flex-col flex-1 justify-between">
                  ...
                  <div className="flex items-center justify-between text-xs text-gray-400 mt-2">
                    ...
                    <span className="flex items-center gap-1">
                      <Layers className="w-3.5 h-3.5" /> {series.totalSeasons} مواسم
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div className="glass-card p-12 text-center rounded-2xl text-gray-400">
            لا توجد مسلسلات متاحة في المكتبة حالياً.
          </div>
        )}
      </div>
    </div>
